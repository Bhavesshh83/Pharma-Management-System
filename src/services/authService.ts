...
// Add request deduplication to prevent duplicate auth calls
const pendingRequests = new Map<string, Promise<any>>();

export const authService = {
  async login({ email, password, role }: LoginData): Promise<AuthResult> {
    const requestKey = `login_${email}`;
    ...
    pendingRequests.set(requestKey, loginPromise);
    return loginPromise;
  },
  ...
  async register(userData: RegisterData): Promise<AuthResult> {
    ...
    pendingRequests.set(requestKey, registerPromise);
